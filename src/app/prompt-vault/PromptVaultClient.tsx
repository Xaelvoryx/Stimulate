...
export function PromptVaultClient({ data }: { data: PromptVaultDataset }) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-2 text-gray-900">Prompt Vault</h1>
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 bg-white text-gray-900 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
          />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredPrompts.map((prompt) => (
          <div
            key={prompt.id}
            className="border border-gray-200 bg-white p-4 cursor-pointer hover:border-blue-500 hover:shadow-md transition-all"
            onClick={() => setSelectedPrompt(prompt)}
          >
            <h3 className="font-semibold text-lg mb-2 text-gray-900">{prompt.title}</h3>
            <p className="text-gray-600 text-sm mb-3 line-clamp-2">
              {prompt.description}
            </p>
            
            <div className="flex flex-wrap gap-1 mb-2">
              <span className="px-2 py-1 bg-blue-50 text-blue-700 text-xs border border-blue-200">
                {prompt.category}
              </span>
              <span className="px-2 py-1 bg-green-50 text-green-700 text-xs border border-green-200">
                {prompt.difficulty}
              </span>
            </div>
            ...
            <div className="flex flex-wrap gap-1">
              {prompt.tags.slice(0, 3).map((tag) => (
                <span key={tag} className="px-2 py-1 bg-gray-50 text-gray-700 text-xs border border-gray-200">
                  {tag}
                </span>
              ))}
              {prompt.tags.length > 3 && (
                <span className="px-2 py-1 bg-gray-50 text-gray-700 text-xs border border-gray-200">
                  +{prompt.tags.length - 3}
                </span>
              )}
            </div>
      ...
      {selectedPrompt && (
        ...
        >
          <div
            className="bg-white border border-gray-200 max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6"
          ...
          >
            <div className="flex justify-between items-start mb-4">
              ...
              <button
                onClick={() => setSelectedPrompt(null)}
                className="text-gray-500 hover:text-gray-700 px-3 py-1 border border-gray-300 hover:border-gray-400"
              >
                Close
              </button>
            </div>
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              <span className="px-3 py-1 bg-blue-50 text-blue-700 border border-blue-200">
                {selectedPrompt.category}
              </span>
              <span className="px-3 py-1 bg-green-50 text-green-700 border border-green-200">
                {selectedPrompt.difficulty}
              </span>
              <span className="px-3 py-1 bg-purple-50 text-purple-700 border border-purple-200">
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              {selectedPrompt.tags.map((tag) => (
                <span key={tag} className="px-3 py-1 bg-gray-50 text-gray-700 border border-gray-200 text-sm">
                  {tag}
                </span>
              ))}
            </div>
            ...
            <div className="mb-4">
              ...
              <pre className="bg-gray-50 border border-gray-200 p-4 overflow-x-auto whitespace-pre-wrap text-gray-800">
