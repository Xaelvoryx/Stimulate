...
async function cloneAndExtract() {
  ...
  for (const [tierName, repos] of Object.entries(config.tiers)) {
    ...
    for (const repoUrl of repos) {
      ...
      try {
        ...
        // Commit this repository's extraction
        await commitRepoExtraction(repoName, prompts);
  ...
  console.log('Saved all prompts to output/json/prompts.json');
}
...
async function commitRepoExtraction(repoName, prompts) {
  ...
  try {
    ...
    console.error(`Failed to commit ${repoName}:`, error.message);
  }
}

cloneAndExtract().catch(console.error);
